...
public class TabellHaug<T extends Comparable<T>> {
	...
	private void reparerOpp() {
		...
		int denne = antall - 1;
		int forelder = (denne - 1) / 2;
		while (denne > 0 && !ferdig) {
			// Har flere noder lenger nede // Har funnet det "minste" av barna. Sml med
			// forelder
			if ((data[denne]).compareTo(data[forelder]) >= 0)
				ferdig = true;
			else { // Bytt om og gå videre nedover hvis forelder er for stor
				hjelp = data[denne];
				data[denne] = data[forelder];
				data[forelder] = hjelp;
				denne = forelder;
				forelder = (denne - 1) / 2;
			}
	...
	private void reparerNed() {
		...
		while ((vbarn < antall) && !ferdig) {
			// Har flere noder lenger nede minbarn = vbarn;
			...
			if ((data[forelder]).compareTo(data[minbarn]) <= 0) {
				...
				// Bytt om og gå videre nedover hvis forelder er for stor
				hjelp = data[minbarn];
	...
	public void skrivTab() { // Hjelpemetode til test
		for (int i = 0; i < antall; i++)
